...
<%
if vExp="xls" then
    Response.AddHeader "content-disposition", "attachment; filename=Reporte_Visitas_" & year(now) &"-"& month(now) &"-"& day(now) & "_" & hour(now) & "_" & minute(now) & ".xls"
	...
	<%if vExp<>"xls" then %>
		...
		<table width="79%" border="0" align="center" cellpadding="0" cellspacing="0">
           ...
           <div class="form-actions">
              <button type="submit" class="btn btn-success" onClick="javascript:irA(document.forms.form1_crit,'graf_visitas.asp?opc=sch');">Buscar</button>
			  <button type="submit" class="btn btn-success" onClick="irAFuera(document.forms.form1_crit,'graf_visitas.asp?opc=sch&exp=xls','_blank')">Exportar</button>

            </div>
	<%else%>
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr class="tituloazul">
		<td colspan="6" align="center"><div align="center">Reporte de Visitas</div></td>
	</tr>
</table>
<%end if%>
          </form>
							...
							<div class="col-sm-12">
								...
								<%else%>
									...
									<table id="demo-foo-pagination" class="table table-bordered table-striped with-check" data-page-size="5">
										...
										<tbody>
											<%											
											sql="set dateformat ymd; exec INFORMEVISITAS_Excel "
											if request.form("fecha_ini") = "" then
												sql=sql & " '2017-01-01', "
											else
												sql=sql & " '" & request.form("fecha_ini") & "', "
