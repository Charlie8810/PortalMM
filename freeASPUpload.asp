...
Class FreeASPUpload
	...
	'Cambios CMV
        if InStr(Right(fileItem.FileName,4),".jpg") or InStr(Right(fileItem.FileName,4),".png") or InStr(Right(fileItem.FileName,4),".jpeg") or InStr(Right(fileItem.FileName,5),".docx") or InStr(Right(fileItem.FileName,5),".xlsx") then
           ...
           location.href = "./carga_archivos.asp?msg=1"
			  </script>
			  <%
		else
		     %>
			  <script>
			     mostrarMensaje('Formato de archivo no permitido. Formatos permitidos: JPG-PNG-JPEG-GIF', 'success');
			  </script>
			  <%
		end if
		Next
